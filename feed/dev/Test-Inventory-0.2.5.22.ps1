...
if ($manifest.releaseVersion -ne '0.2.5.22' -or $manifest.minimumUpdaterVersion -ne '0.1.4.0' -or
    $manifest.components.hwmServiceVersion -ne '0.2.5.17' -or $manifest.components.hwmTrayVersion -ne '1.0.0.14') { throw 'The DEV feed is not the reviewed inventory release. Stop and review the new feed.' }
...
$status | Format-List
if ($status.LastInstalledReleaseVersion -ne '0.2.5.22') { throw 'Expected release was not installed.' }
...
$evidence = Join-Path $data ('inventory-0.2.5.22-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.json')
