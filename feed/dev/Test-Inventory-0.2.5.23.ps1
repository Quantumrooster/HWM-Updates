...
if ($manifest.releaseVersion -ne '0.2.5.23' -or $manifest.minimumUpdaterVersion -ne '0.1.4.0' -or
    $manifest.components.hwmServiceVersion -ne '0.2.5.17' -or $manifest.components.hwmTrayVersion -ne '1.0.0.15') { throw 'The DEV feed is not the reviewed inventory release. Stop and review the new feed.' }
...
if ([version](Get-Item (Join-Path $hwm 'HeadlineWorkstationService.exe')).VersionInfo.FileVersion -gt [version]'0.2.5.17' -or
    [version](Get-Item (Join-Path $hwm 'HeadlineWorkstationMonitor.exe')).VersionInfo.FileVersion -gt [version]'1.0.0.15') { throw 'Newer HWM components are installed; refusing this test.' }
...
$status | Format-List
if ($status.LastInstalledReleaseVersion -ne '0.2.5.23') { throw 'Expected release was not installed.' }
...
$evidence = Join-Path $data ('inventory-0.2.5.23-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.json')
