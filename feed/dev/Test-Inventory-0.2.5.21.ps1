...
if ($manifest.releaseVersion -ne '0.2.5.21' -or $manifest.minimumUpdaterVersion -ne '0.1.4.0' -or
    $manifest.components.hwmServiceVersion -ne '0.2.5.16' -or $manifest.components.hwmTrayVersion -ne '1.0.0.14') { throw 'The DEV feed is not the reviewed inventory release. Stop and review the new feed.' }
$hwm = Join-Path $env:ProgramFiles 'Headline\Workstation Monitor'
if ([version](Get-Item (Join-Path $hwm 'HeadlineWorkstationService.exe')).VersionInfo.FileVersion -gt [version]'0.2.5.16' -or
...
$status | Format-List
if ($status.LastInstalledReleaseVersion -ne '0.2.5.21') { throw 'Expected release was not installed.' }
...
do {
    ...
    try {
        if ((Get-Item $healthPath).LastWriteTimeUtc -ge $started) {
            ...
            if ($candidate.AgentVersion -eq '0.2.5.16' -and $candidate.UpdateInventoryAvailable -eq $true -and $candidate.WindowsUpdateAvailable -eq $true) { $health = $candidate }
        }
...
$evidence = Join-Path $data ('inventory-0.2.5.21-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.json')
